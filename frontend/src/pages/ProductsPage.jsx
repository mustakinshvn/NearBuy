...
import { ShoppingBag, Filter, Search, X, ChevronDown, Star, RefreshCw, XCircle } from 'lucide-react';
...
const ProductsPage = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [allProducts, searchQuery, selectedCategory, priceRange, sortBy, minRating]);
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-slate-50 to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-2">Please wait while we fetch the latest products</p>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600">Browse our collection of {filteredProducts.length} amazing products</p>
          </div>
          <button 
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs text-slate-600">Min: ৳{priceRange[0]}</label>
                    <input
                      type="range"
                      min="0"
                      max="20000"
        ...
        {allProducts.length === 0 && !loading && !error ? (
          <div className="bg-white rounded-3xl shadow-xl p-16 text-center max-w-2xl mx-auto">
            ...
            <h2 className="text-3xl font-bold text-slate-800 mb-3">No Products Available</h2>
            <p className="text-lg text-slate-600 mb-2">We're currently updating our inventory</p>
            <p className="text-slate-500 mb-8">Check back soon for exciting new products!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="/" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105">
                Back to Home
              </a>
              <a href="/shops" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-white border-2 border-slate-300 text-slate-700 rounded-xl font-semibold hover:border-blue-600 hover:text-blue-600 transition-all">
                Browse Shops
              </a>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((product) => (
              <ProductCard key={product.product_id} product={product} mode="grid" onAddToCart={handleAddToCart} />
            ))}
          </div>
        ) : (
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center max-w-xl mx-auto">
            ...
            <h3 className="text-2xl font-bold text-slate-700 mb-3">No products match your filters</h3>
            <p className="text-slate-500 mb-6">Try adjusting your search terms or removing some filters</p>
