...
import { Package, Clock, CheckCircle, Truck, XCircle, RefreshCw, ChevronDown, ChevronUp, ShoppingBag } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-purple-50 to-pink-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-2">Please wait while we fetch your order history</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-linear-to-br from-purple-50 to-pink-50">
        <div className="container mx-auto px-4 py-12">
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center max-w-2xl mx-auto">
            ...
            <h2 className="text-2xl font-bold text-red-800 mb-3">Unable to Load Orders</h2>
            <p className="text-red-600 mb-6">{error}</p>
            <div className="flex gap-4 justify-center">
              <ButtonCard onClick={() => window.location.reload()} label="Try Again" />
              <a 
                href="/products"
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {orders.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl p-16 text-center max-w-2xl mx-auto">
            ...
            <p className="text-lg text-slate-600 mb-2">Looks like you haven't placed any orders yet</p>
            <p className="text-slate-500 mb-8">Start exploring our amazing products and create your first order!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="/products" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-linear-to-r from-green-600 to-blue-600 text-white rounded-xl font-semibold hover:from-green-700 hover:to-blue-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105">
                <ShoppingBag className="w-5 h-5" />
                Browse Products
              </a>
              <a href="/" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-white border-2 border-slate-300 text-slate-700 rounded-xl font-semibold hover:border-purple-600 hover:text-purple-600 transition-all">
                Back to Home
              </a>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {orders.map((order) => {
              ...
              return (
                <div key={order.order_id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex items-start gap-4 flex-1">
                      <div className={`${statusColor.bg} w-12 h-12 rounded-lg flex items-center justify-center shrink-0`}>
                        <IconComponent className={`${statusColor.text} w-6 h-6`} />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="font-bold text-slate-800 text-lg">Order #{order.order_id}</h3>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadgeColor(order.order_status)}`}>
                            {order.order_status}
                          </span>
                        </div>
                        <div className="space-y-1 text-sm text-slate-600">
                          ...
                          {order.tracking_number && (
                            <p className="font-medium">Tracking: <span className="text-blue-600">{order.tracking_number}</span></p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="lg:text-right space-y-2">
                      ...
                      {discountAmount > 0 && (
                        <p className="text-sm text-green-600 font-medium">Discount: -৳{discountAmount.toFixed(2)}</p>
                      )}
                      <div className="pt-2 border-t border-slate-200">
                        <p className="text-sm text-slate-600">Final Amount</p>
                        <p className="text-xl font-bold text-blue-600">৳{finalAmount.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700">{order.payment_method}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700">৳{shippingCost.toFixed(2)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Vendor ID</p>
                      <p className="text-sm font-semibold text-slate-700">#{order.vendor_id}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-3">
                    ...
                    {order.order_status === 'Pending' && (
                     <ButtonCard onClick={() => cancelOrder(order.order_id)} label="Cancel Order" className="px-2 py-2 text-white font-bold" /> 
                    )}
                  </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200">
                      ...
                      {loadingItems[order.order_id] ? (
                        <div className="text-center py-4">
                          ...
                          <p className="text-sm text-slate-600 mt-2">Loading items...</p>
                        </div>
                      ) : orderItems[order.order_id]?.length > 0 ? (
                        <div className="space-y-3">
                          {orderItems[order.order_id].map((item) => {
                            const unitPrice = parseFloat(item.discount_price ?? item.unit_price) || 0;
                            const itemTotal = parseFloat(item.total_price) || (item.quantity * unitPrice);
                            return (
                              <div key={item.order_item_id} className="flex items-center gap-4 p-3 bg-slate-50 rounded-lg">
                                {item.product_image ? (
                                  <img 
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-4 mt-1 text-sm text-slate-600">
                                    ...
                                    <span>Unit Price: ৳{(parseFloat(item.unit_price) || 0).toFixed(2)}</span>
                                    {item.discount_price && (
                                      <span className="text-green-600">
                                        Discounted Unit: ৳{(parseFloat(item.discount_price) || 0).toFixed(2)}
                                      </span>
                                    )}
                                  </div>
                                </div>
                        ...
                        <p className="text-sm text-slate-500 py-4 text-center">No items found for this order</p>
                      )}
                    </div>
                  )}
                </div>
